...
public class ExDialog extends ListActivity {
    ...
    public final static int SPECIALTYPE=2;
	@Override
			...
			for (int i = 0; i < files.size(); i++) {
                ...
                switch (Mode)
                {
                    ...
                    case DIRECTORY:
                        ...
                        break;
                    case SPECIALTYPE:
                        String str=DensityUtil.getFilePostfix(((File)(files.get(i))).getName());
                        if (str!=null&&str.equals(this.postFix))
                        {
                            map.put("img", R.drawable.check);
                            list.add(map);
                        }
